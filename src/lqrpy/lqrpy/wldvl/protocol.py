...
class WlProtocolParser(object):
    ...
    def parse(self, sentence):
        ...
        if sop != SOP:
            ...
            raise WlProtocolParseError(
                "Missing SOP: Got {} Expected {}".format(sop, SOP)
            )
        ...
        if direction not in [DIR_CMD, DIR_RESP]:
            raise WlProtocolParseError(
                "Invalid direction {}: {}".format(direction, sentence)
            )
    ...
    def doDict(self, cmd, direction, options):
        ...
        if cmd == 120:
            ...
            result = {
                ...
                "status": True if options[8].decode("utf-8") == "0" else False,
            }
        return result
...
class WlDVL(WlDVLBase):
    ...
    def __init__(self, device, baudrate=115200, debug=False):
        try:
            ...
            # self._serial.write('wcv'.encode('utf-8'))
