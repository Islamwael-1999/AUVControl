...
def simulation():
    sx, sy, sz, syaw, sv, sa = -10, 10.0,10, np.deg2rad(10.0), 1.0, 0.1
    gx, gy, gz, gyaw, gv, ga = 30.0, -10.0,5, np.deg2rad(180.0), 5.0, 0.5
    ...
    for T in np.arange(MIN_T, MAX_T, T_STEP):
        ...
        zqp = QuinticPolynomial(sz, 0.1, 0.1, gz, 0.0, 0.0, T)

        for t in np.arange(0.0, T + dt, dt):
            path.t.append(t)
            path.x.append(xqp.calc_xt(t))
            path.y.append(yqp.calc_xt(t))
            path.z.append(zqp.calc_xt(t))
    ...
    for i in range(len(path.t)):
        ...
        ax.plot3D(path.y,path.x, path.z, 'gray')
