...
def headingToStandard(hdg):
    # Convert NED heading to standard unit cirlce...degrees only for now ()
...
def calcDubinsPath(wpt1, wpt2, vel, phi_lim):
    ...
    #d is eculidian distance between intial and final point
    #D is actual distance between 2 points 
...
#time step function that returrns array of points of the path
def dubins_traj(param,step):
    ...
    print("lenght of the path",length)
...
def main():
    ...
    pt1 = Waypoint(0,0,0)
    # pt2 = Waypoint(1000,1000,100)
    # pt3 = Waypoint(2000,2000,180)
    # pt4 = Waypoint(30,30,0)
    
    # pt4 = Waypoint(20000,20000,270)
    # pt5 = Waypoint(5000,20000,300)

    # pt1 = Waypoint(0,0,0)
    # pt2 = Waypoint(5000,5000,0)
...
def calculatePath(Wptz): 
    # Run the code
    i = 0
    trajectory=[]
    while i<len(Wptz)-1:                            #0.3  0.01 
        param = calcDubinsPath(Wptz[i], Wptz[i+1], 0.3, 0.05)
        path = dubins_traj(param,1)
        
        x,y,psi = Wptz[i+1].x,Wptz[i+1].y,Wptz[i+1].psi*math.pi/180
        trajectory=[*trajectory,*path,[x,y,psi]]
        # np.concatenate(paths,path)
        # print("path")
        # print(path[0])
        # print(path[100])
        ...
        # # x-points of the path
        # print("x-points")
        # print(path[:,0])
        # # y-points of the path
        # print("y-points")
        # print(path[:,1])
        # #yaw of the path
        # print("yaw-points")
        ...
        # Plot the results
        # plt.plot(Wptz[i].y,Wptz[i].x,'kx')
        # plt.plot(Wptz[i+1].y,Wptz[i+1].x,'kx')
